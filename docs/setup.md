...
Docker is the intended path for running Zyntro because it starts the frontend, backend, worker, PostgreSQL, and Redis together with the expected wiring.
...
With Docker, Zyntro stores persistent data in named volumes:
...
Zyntro defaults to self-host mode:
