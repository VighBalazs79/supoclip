...
docker exec -i Zyntro-postgres psql -U postgres -d Zyntro < backend/migrations/001_add_progress_fields.sql
```
...
services:
  Zyntro-worker:
    ...
    environment:
      - DATABASE_URL=postgresql://postgres:postgres@postgres:5432/Zyntro
      - REDIS_HOST=redis
      - REDIS_PORT=6379
...
docker-compose logs -f Zyntro-worker

# Or check Redis queue status:
docker exec -it Zyntro-redis redis-cli
> KEYS arq:*
> LLEN arq:queue
```
...
```bash
docker-compose ps Zyntro-worker
docker-compose logs Zyntro-worker
```

2. Check Redis connection:
```bash
docker exec Zyntro-backend .venv/bin/python -c "import redis; r=redis.Redis(host='redis'); print(r.ping())"
```
