...
const databaseUrl =
  process.env.DATABASE_URL ?? "postgresql://Zyntro:Zyntro_password@127.0.0.1:5432/Zyntro";
const backendDatabaseUrl =
  process.env.TEST_DATABASE_URL ??
  (databaseUrl.startsWith("postgresql+asyncpg://")
    ? databaseUrl
    : databaseUrl.replace("postgresql://", "postgresql+asyncpg://"));
const backendAuthSecret = process.env.BACKEND_AUTH_SECRET ?? "Zyntro_test_secret";
  ...
  webServer: [
    ...
    },
    {
      ...
      env: {
        ...
        BETTER_AUTH_SECRET:
          process.env.BETTER_AUTH_SECRET ?? "Zyntro_better_auth_test_secret",
      },
    },
  ],
});
